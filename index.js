...
/**
 * Return the `xpSessionStore` extending `express`'s session Store.
 *
 * @param {Object} options Optional
 */
class xpSessionStore extends Store {
    constructor(options = {}) {
        super(options)
        this._opts = Object.assign({}, defOpts, options)
        this._db = this._opts && this._opts.hasOwnProperty("dbname") ? mongoose.connection.useDb(this._opts.dbname) : mongoose.connection
        this._client = this._db.model(this._opts.collection, Schema)
    }
    get(_id, callback) {
    ...
    set(_id, session, callback) {
        try {
            ...
            for (const key in session) {
                if (key === "cookie") {
                    _session[key] = session[key].toJSON ? session[key].toJSON() : session[key]
                } else {
    ...
    touch(...args) { return this.set(...args) }
